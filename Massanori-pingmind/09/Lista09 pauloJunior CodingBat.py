...
def same_first_last(nums):
  return True if len(nums) != 0 and nums[0] == nums[len(nums)-1] else False
...
def maior_ponta(nums):
	maior = nums[0] if nums[0] >= nums[len(nums)-1] else nums[len(nums)-1]
	nova = []
	for i in range(len(nums)): nova.append(maior)
	return nova
...
# caso seja verão, então a temperatura superior à 100 no lugar de 90
