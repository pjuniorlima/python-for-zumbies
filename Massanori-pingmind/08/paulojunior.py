...
def count_verbs_txt(txtList):
	...
	for i in txtList:
		if len(i) == 7 and i[-1] not in 'zmb':
			...
			if i[0] not in 'zmb':
				firstpersonverb += 1
	return verbcount, firstpersonverb

def sort_by(txtList):
	str_order = 'zmbtshjpnwlrcxkqvdgf'
	return sorted(txtList, key=lambda sort: [str_order.index(c) for c in sort])
	
def respostas():
	print('A;'+str(check_prep_txt(txtB)))
	print('B;'+str(count_verbs_txt(txtB)[0]))
	print('C;'+str(count_verbs_txt(txtB)[1]))
	print('D;'+str(sort_by(txtB)))
