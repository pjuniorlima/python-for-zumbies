#!/usr/bin/python3 -tt
# Exercí­cios by Nick Parlante (CodingBat)
...
def near_ten(n):
	dist = n%10
	return True if (dist+10) <= 12 or abs((dist-10)) <= 2 else False
...
def count_code(s):
	positionco = s.find('co', 0)
	codecount = 0
	i = 0
	while i < len(s)-2:
		if i+3 < len(s) and s[i]+s[i+1] == 'co' and s[i+3] == 'e':
			codecount += 1
			i += 3
		else:
			i += 1
...
def end_other(a, b):
	a = a.lower()
	b = b.lower()
	return True if a in b[len(b)-len(a):] or b in a[len(a)-len(b):] else False
...
def soma_na_lista(n, lista):
	for i in range(len(lista)):
		for j in range(1,len(lista)):
			if n == lista[i]+lista[j] and lista[i] != lista[j]:
				return True
	return False
	#return
...
def fila_tijolos(n_peq, n_gra, meta):
	if n_peq >= meta or (meta%5 == 0 and 5*n_gra >= meta):
		return True
	elif (meta-n_peq*1)%5 == 0 and 5*n_gra >= (meta-n_peq*1):
		return True
	else:
		temp = meta - n_gra*5
		if 1*n_peq >= temp and not temp < 0:
			return True
		else:
			return False
